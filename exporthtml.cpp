...
void tableHTML(vector<string> th, vector<string> col, vector<vector<double>> data, ofstream& dataOut) {
...
void tdTable(vector<vector<double>> data, vector<string> col, ofstream& dataOut) {
	for (int i = 0; i < data.size(); i++) {
		...
		for (int j = 0; j < data[i].size(); j++) {
			...
			dataOut << toStr(data[i][j]) << "</th>" << endl;
		}
		dataOut << "\t\t</tr>" << endl;
	}
}
